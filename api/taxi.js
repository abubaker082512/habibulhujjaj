...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      let query = supabase.from('taxi_services').select('*').order('created_at', { ascending: false });
  ...
  if (req.method === 'POST') {
    try {
      const body = req.body || {};
      const { data, error } = await supabaseAdmin.from('taxi_services').insert([body]).select();
      if (error) throw error;
  ...
  if (req.method === 'PUT') {
    ...
    try {
      const body = req.body || {};
      const { data, error } = await supabaseAdmin.from('taxi_services').update(body).eq('id', id).select();
      if (error) throw error;
  ...
  if (req.method === 'DELETE') {
    if (!id) return res.status(400).json({ message: 'ID is required' });
    try {
      const { error } = await supabaseAdmin.from('taxi_services').delete().eq('id', id);
      if (error) throw error;
